...
class AutogeneratedRules(BaseModel):
    rule_id: int
    title: str
    description: Optional[str]
    rule: str
...
# Endpoint to get all autogenerated rules
@app.get("/autogenerated-rules", response_model=List[AutogeneratedRules], status_code = status.HTTP_200_OK)
async def get_autogeneratedrules(rule_id: int = None):
    print(f'rule_id: {rule_id}')
    if rule_id:
        print('Getting filtered rules')
        query = autogeneratedrules.select().where(autogeneratedrules.c.rule_id == rule_id)
        return await database.fetch_all(query)
    else:
        print('Getting all rules')
        query = autogeneratedrules.select()
        return await database.fetch_all(query)
